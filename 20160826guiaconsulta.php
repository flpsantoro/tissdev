...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$validadeCarteira = $_POST['dataAtendimento']; //$_POST['validadeCarteira'];
		...
		$cbo =  $_POST['cbo'];
		$nomeProfissionalExec = ($_SESSION['nm_prestador'] == 'F') ? $_SESSION['nomefantasia'] : '';
		...
		$stmt = $conn->prepare("insert into TISS_GUIA_CONSULTA_DIG (NR_GUIA,                   
						...
						values
											...
											:SN_ATENDIMENTO_RN,
											:VL_CONSULTA,
											TO_DATE(:DT_VALIDADE_CARTEIRA,'YYYY-MM-DD'))");
		...
		$stmt->bindValue(':NM_PROFISSIONAL_EXECUTANTE', ($_SESSION['nm_prestador'] == 'F') ? $_SESSION['nomefantasia'] : '' );
		...
		$stmt->bindValue(':CD_UF_CONSELHO',$uf);
		...
		$stmt->bindValue(':SN_ATENDIMENTO_RN',$atendimentoRN);
...
<div id="guiaConsulta" style="height:1300px;" class="form-group" >
	<form id="login" name="login" method="post" onSubmit="return validate('');" autocomplete="off" action="index.php?pagina=guiaconsulta"> 
		<div id="node1" >
		...
		<div id="node31">
			...
			<select name="lote" id="lote" style="width:165px;font-size:small;" autofocus tabindex="1">
				...
				<?php	
				foreach ($conn->query("SELECT DISTINCT(NR_LOTE) FROM TISS_TRANSACAO_PROTOCOLO_DIG WHERE NR_PROTOCOLO_RECEBIMENTO is null AND CPF_CGC =" .$_SESSION['cpf_cnpj'] . " ORDER BY NR_LOTE") as $row2) { ?>
		...
		<div id="node7">
			<input type="text"  size="1" style="text-align:center;" maxlength="1" name="atendimentoRN" id="atendimentoRN"  tabindex="7" value="<?php print isset($_SESSION['dados']['SN_ATENDIMENTO_RN']) ? $_SESSION['dados']['SN_ATENDIMENTO_RN'] : ''; ?>" onkeydown="validaAtendimentoRN(event);"  />
		</div>
		...
		<div id="node13">
			<input type="text"  size="55" maxlength="55" name="nomeProfissionalExec" tabindex="10" value="<?php print $_SESSION['tp_credenciado'] == 'F' ? $_SESSION['nomefantasia'] : '' ?>" <?php print $_SESSION['tp_credenciado'] == 'F' ? 'DISABLED' : '' ?>/>
		</div><!-- #node13 end -->

		<div id="node15">
			<input type="text"  size="20" maxlength="15" name="numeroConselho" tabindex="12" value="<?php print $_SESSION['tp_credenciado'] == 'F' ? $_SESSION['nr_crm'] : '' ?>"/>
		</div>

		<div id="node16">
			<input type="text"  size="2" maxlength="2" name="uf" tabindex="13"  value="<?php print $_SESSION['tp_credenciado'] == 'F' ? $_SESSION['cd_uf_crm'] : '' ?>"/>
		</div><!-- #node16 end -->

		<div id="node14">
			<input type="text"  size="5" maxlength="5" name="conselhoProfissional" tabindex="11"  value="<?php print $_SESSION['tp_credenciado'] == 'F' ? 'CRM' : '' ?>" disabled/>
		...
		<div id="node24">
			<input type="text"  size="20" value="10101012" disabled/>
		...
		<div id="node27" tabindex="20">
			<textarea rows="7" cols="150" style="background:#d1d0d0;width:1174px"  maxlength="240" 
id="observacao" name="observacao"><?php print isset($_SESSION['dados']['DS_OBS']) ? $_SESSION['dados']['DS_OBS'] : ''; ?></textarea>
		</div>
...
function validate(botao) 
{
    ...
    {	
	    ...
	    {      
	    	strMsg += '*Guia Atribuído pela Operadora é obrigatória.<br>';
	    }
	 
	    if(!vNrCarteira)
	    {
	    	strMsg += '*Número da Carteira é obrigatório.<br>';
	    }
	    
	    if(!vCnpjCpf)
	    {
			strMsg += '*CNPJ/CPF é obrigatório.<br>';
	    }

	    if(!vNumeroConselho)
	    {
	    	strMsg += '*Conselho Profissional é obrigatório.<br>';
	    }

	    if(!vUf)
	    {
	    	strMsg += '*UF é obrigatório.<br>';
	    }

	    if(!vTipoConsulta)
	    {
	    	strMsg += '*Tipo Consulta é obrigatório.<br>';
	    }

	    /*
	    if(!vNrGuiaPrestador)
	    {
	    	strMsg += '*Guia Prestador é obrigatório.<br>';
	    } 
	    */  

	    if(vcbo == "-")
	    {
	    	strMsg += '*Código CBO é obrigatório.<br>';
	    }   
