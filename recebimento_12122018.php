...
if (!empty($_POST) || !is_null($nr_protocolo)) {
	...
	if (!is_null($nr_protocolo)) {
		$sql = "SELECT t.cpf_cgc,
					   t.nr_protocolo_recebimento,
					   t.nr_lote,
					   count(g.nr_guia) nr_guias,
					   decode(t.cd_status_protocolo, 0, 'Erro', 1, 'Aguardando', 2, 'Em Análise', 3, 'Finalizado', 4, 'Excluído') ds_status,
					   to_char(t.dt_recebimento, 'dd/mm/yyyy') dt_recebimento,
					   'guias' as pagina
				  from tiss_transacao_protocolo t,
					   tiss_guia_consulta g
				 where t.nr_protocolo_recebimento = " . intval($nr_protocolo) ." 
				   and t.nr_seq_importacao = g.nr_seq_importacao
			  group by t.cpf_cgc,
					   t.nr_protocolo_recebimento,
					   t.nr_lote,
					   t.cd_status_protocolo,
					   t.dt_recebimento ";
		...
		$sql.= " union 
				select t.cpf_cgc,
					   ...
					   t.nr_lote,
					   count(g.nr_guia) nr_guias,
					   decode(t.cd_status_protocolo, 0, 'Erro', 1, 'Aguardando', 2, 'Em Análise', 3, 'Finalizado', 4, 'Excluído', 5, 'Aguardando') ds_status,
					   to_char(t.dt_recebimento, 'dd/mm/yyyy') dt_recebimento,
					   'guias_dig' as pagina
				  from tiss_transacao_protocolo_dig t,
...
<?php
	...
	foreach ($conn->query($sql . $where) as $row) {
			...
			var_dump("-----");
			var_dump($row['DS_STATUS']);
			var_dump("-----");

			if ($row['DS_STATUS'] == 'Em Análise') {
			...
			<td>
				<form method="get" action="?pagina=recebimento">
					...
					<select name="nr_protocolo" style="width: 100px;">
						<?php $sql='SELECT NR_PROTOCOLO_RECEBIMENTO
						              FROM TISS_TRANSACAO_PROTOCOLO
							         WHERE CD_STATUS_PROTOCOLO <> 0
									   AND CPF_CGC = '. $row['CPF_CNPJ'].'
								     UNION
									SELECT NR_PROTOCOLO_RECEBIMENTO
									  FROM TISS_TRANSACAO_PROTOCOLO_DIG
									 WHERE CD_STATUS_PROTOCOLO <> 0
									   AND CPF_CGC = '. $row['CPF_CNPJ'] .'
							      ORDER BY NR_PROTOCOLO_RECEBIMENTO DESC';
