...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		*/
		...
		if ($qtdGuia != 0)
		{
			 ...
			 #FALTA >>> A.DT_VALIDADE_CARTEIRA = TO_DATE(:DT_VALIDADE_CARTEIRA,'DD/MM/YYYY'),
			 #$conn->rollback();
			 #echo '<font color="red">====>>>> ...cpf_cgc... ', $CnpjCpf ,' GUIA ', $nrGuiaAtrOperador ,'</font>' ."\n";
			 #echo '<font color="red">====>>>> SN_ATEND ', strtoupper($atendimentoRN) ,' OBS ', $observacao ,'</font>' ."\n";
			 
			 /*$sqlALTERAGUIA = "UPDATE TISS_GUIA_CONSULTA_DIG A
                     SET A.NR_CARTEIRA       = :NR_CARTEIRA,
                         A.SN_ATENDIMENTO_RN = :SN_ATENDIMENTO_RN,
                         A.NR_CNS            = :NR_CNS,
                         A.CD_CNES           = :CD_CNES,
                         A.CD_CONSELHO       = :CD_CONSELHO,
                         A.NR_CONSELHO       = :NR_CONSELHO,
                         A.CD_UF_CONSELHO    = :CD_UF_CONSELHO,
                         A.CD_CBO            = :CD_CBO,
                         A.CD_INDICADOR_ACIDENTE = :CD_INDICADOR_ACIDENTE,
                         A.DT_EVENTO         = TO_DATE(:DT_EVENTO,'YYYY-MM-DD'),
                         A.CD_TIPO_CONSULTA  = :CD_TIPO_CONSULTA,
                         A.CD_ESPECIALIDADE  = :CD_ESPECIALIDADE,
                         A.VL_CONSULTA       = :VL_CONSULTA,
                         A.DS_OBS            = :DS_OBS
                   WHERE A.CPF_CGC = :CPF_CGC
  		               AND A.NR_GUIA = :NR_GUIA";*/
			$sqlALTERAGUIA = "UPDATE TISS_GUIA_CONSULTA_DIG A
								 SET A.NR_CARTEIRA       = :NR_CARTEIRA,
									 A.SN_ATENDIMENTO_RN = :SN_ATENDIMENTO_RN,
									 A.NR_CNS            = :NR_CNS,
                               ...
                               WHERE A.CPF_CGC = :CPF_CGC
  		                         AND A.NR_GUIA = :NR_GUIA";
			$stmtALTERAGUIA = $conn->prepare($sqlALTERAGUIA);
			#echo '<font color="red">====>>>> ... teste 111 de sql >>>>. ', $sqlALTERAGUIA ,'</font>' ."\n";
			#$stmtALTERAGUIA->bindValue(':SN_ATENDIMENTO_RN',strtoupper($atendimentoRN));
			#$stmtALTERAGUIA->bindValue(':DS_OBS',$observacao);
			$stmtALTERAGUIA->bindValue(':NR_CARTEIRA',$nrCarteira);
			$stmtALTERAGUIA->bindValue(':SN_ATENDIMENTO_RN',strtoupper($atendimentoRN));
			$stmtALTERAGUIA->bindValue(':NR_CNS',$nrCartaoNacionalSaude);
			$stmtALTERAGUIA->bindValue(':CD_CNES',$codigoCNES);
			//$stmtALTERAGUIA->bindValue(':CD_CONSELHO', $conselhoProfissional);
			$stmtALTERAGUIA->bindValue(':CD_CONSELHO', 5265377);
			$stmtALTERAGUIA->bindValue(':NR_CONSELHO',$numeroConselho);
			$stmtALTERAGUIA->bindValue(':CD_UF_CONSELHO',strtoupper($uf));
			$stmtALTERAGUIA->bindValue(':CD_CBO',$cbo);
			$stmtALTERAGUIA->bindValue(':CD_INDICADOR_ACIDENTE',$indAcidente);
			$stmtALTERAGUIA->bindValue(':DT_EVENTO',$dataAtendimento);                
			$stmtALTERAGUIA->bindValue(':CD_TIPO_CONSULTA',$tipoConsulta);
			$stmtALTERAGUIA->bindValue(':CD_ESPECIALIDADE',$cdEspecialidade);        
			$stmtALTERAGUIA->bindValue(':VL_CONSULTA',$valorProcedimento);
			$stmtALTERAGUIA->bindValue(':DS_OBS',$observacao);
			#
			$stmtALTERAGUIA->bindValue(':CPF_CGC',$CnpjCpf);
			$stmtALTERAGUIA->bindValue(':NR_GUIA',$nrGuiaAtrOperador);
			$stmtALTERAGUIA->execute();
			$conn->commit();
		//echo '<font color="red">====>>>> ValidadeCarteira Novamente: ',  $validadeCarteira, '</font>' ."\n";
		//$stmtALTERAGUIA->bindValue(':DT_VALIDADE_CARTEIRA',$validadeCarteira);
		#$stmt = $db_con->prepare($sql);            
        #$stmt->execute();
		/*
			$stmtALTERAGUIA = $conn->prepare($sqlALTERAGUIA);
			$stmtALTERAGUIA->execute(array(':CPF_CGC' => $CnpjCpf,
   		                               ':NR_GUIA' => $nrGuiaAtrOperador));
		*/
...
<div id="guiaconsulta_fabio" style="height:1300px;" class="form-group" >
	<form id="login" name="login" method="post" onSubmit="return validate('');" autocomplete="off" action="index.php?pagina=guiaconsulta_fabio"> 
		...
		<div id="node26">
			...
			<input type="number" min="1" step="0.01"  size="8" maxlength="8"  placeholder='0.00'   name="valorProcedimento" tabindex="19" value="<?php print isset($_SESSION['dados']['VL_CONSULTA']) ? $_SESSION['dados']['VL_CONSULTA'] : ''; ?>"/>
