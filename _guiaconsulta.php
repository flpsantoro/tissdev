	...
	<form id="login" name="login" method="post" onSubmit="return validate('');" autocomplete="off" action="index.php?pagina=guiaconsulta"> 
		...
		<div id="node31">
			...
			<select name="lote" id="lote" style="width:165px;font-size:small;" autofocus tabindex="1">
				<option value="-">Novo xLote</option>				
				<?php	
				foreach ($conn->query("SELECT DISTINCT(NR_LOTE) FROM TISS_TRANSACAO_PROTOCOLO_DIG WHERE NR_PROTOCOLO_RECEBIMENTO is null AND CPF_CGC =" .$_SESSION['cpf_cnpj'] . " ORDER BY NR_LOTE ASC") as $row2) { ?>
		...
		<div id="node7">
			<input type="text"  size="1" style="text-transform:uppercase;" maxlength="1" name="atendimentoRN" id="atendimentoRN"  tabindex="7" value="<?php print isset($_SESSION['dados']['SN_ATENDIMENTO_RN']) ? $_SESSION['dados']['SN_ATENDIMENTO_RN'] : ''; ?>" onkeydown="validaAtendimentoRN(event);"  />
		</div>
		...
		<div id="node14">
			<input type="text"  size="5" maxlength="5" name="conselhoProfissional" tabindex="11"  value="<?php print $_SESSION['tp_credenciado'] == 'F' ? 'CRM' : '' ?>" enable/>
		...
		<div id="node22">
			<select name="tipoConsulta" id="tipoConsulta" style="width:120px;font-size:small;" tabindex="18">
				<!-- #node22 end //<option value="0">    -    </option> -->
