...
<div id="guiaconsulta" style="height:1300px;" class="form-group" >
	<form id="login" name="login" method="post" onSubmit="return validate('');" autocomplete="off" action="index.php?pagina=guiaconsulta"> 
		...
		<div id="node14">
			<select name="conselhoProfissional" id="conselhoProfissional" style="width:80px;font-size:small;" tabindex="14">
				<option value="-"> - </option>				
				<?php	
				foreach ($conn->query("SELECT CD_CONSELHO, DS_CONSELHO FROM TISS_CONSELHO_PROFISSIONAL WHERE CD_CONSELHO <> 'XXXXXXX'") as $row) { ?>
						<option value="<?php print $row['CD_CBO'] ?>" 
						<?php if((isset($_SESSION['dados']['CD_CONSELHO'])) && ($_SESSION['dados']['CD_CONSELHO'] == $row['CD_CONSELHO'])) { print('SELECTED');}?>><?php print $row['CD_CONSELHO']. ' - '.$row['DS_CONSELHO'] ?></option>
				<?php } ?>
			</select>
