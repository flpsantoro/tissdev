...
if (isset($_POST['botao']) && $_POST['botao'] == 'Salvar') {
    try {
        ...
        $_SESSION['dados']['DATA_ATENDIMENTO'] = $_POST['dataAtendimento']; //$_POST['validadeCarteira'];
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if ($nrLote == "-") {
            ...
            $row = $stmt->fetchAll();
            $nrLote = $nrLote + 1;
            #echo '<font color="red">====>>>> $nrLote ',$nrLote, '</font>' ."\n";
        }
        #--------------------------------------------------------------------------------------------------
...
if ((isset($_POST['botao']) && $_POST['botao'] == 'Consultar') || (isset($_GET['guia'])) || isset($_GET['seqimportacao'])) {
    ...
    #echo '<font color="red">====>>>> CPF_CGC: ',  $_SESSION['cpf_cnpj'] , 'SeqImportacao: ', $seqImportacao, 'Guia: ' , $_POST['nrGuiaAtrOperador'] ,  '</font>' ."\n";
        ...
        $rows = $conn->query(
            sprintf(/*"SELECT
			  ...
			  WHERE P.NR_SEQ_IMPORTACAO = G.NR_SEQ_IMPORTACAO
                AND G.NR_CARTEIRA       = B.NR_CARTEIRA
				AND P.CPF_CGC           = '%s'
...
<div id="guiaconsulta" style="height:1300px;" class="form-group">
    <form id="login" name="login" method="post" onSubmit="return validate('');" autocomplete="off"
          action="index.php?pagina=guiaconsulta">
        <div id="node1">
            ...
            <select name="lote" id="lote" style="width:165px;font-size:small;" autofocus tabindex="1">
                ...
                foreach ($conn->query("SELECT DISTINCT(NR_LOTE) FROM TISS_TRANSACAO_PROTOCOLO_DIG WHERE NR_PROTOCOLO_RECEBIMENTO is null AND CPF_CGC =" . $_SESSION['cpf_cnpj'] . " ORDER BY NR_LOTE DESC") as $row2) { ?>
                    <option value="<?php print $row2['NR_LOTE'] ?>"
                        <?php
                        if ((isset($_SESSION['dados']['NR_LOTE'])) && ($_SESSION['dados']['NR_LOTE'] == $row2['NR_LOTE']) || ($_SESSION['nrLote'] == $row2['NR_LOTE'])) {
                            print('SELECTED');
                        } ?> ><?php print $row2['NR_LOTE'] ?></option>
                <?php } ?>
        ...
        <div id="node2">
            <input type="text" size="20" maxlength="20" name="nrGuiaAtrOperador" id="nrGuiaAtrOperador" tabindex="2"
                   value="<?php print isset($_SESSION['dados']['NR_GUIA']) ? $_SESSION['dados']['NR_GUIA'] : ''; ?>"/>
        </div><!-- #node2 end -->

        <div id="node3">
            <input type="text" size="20" maxlength="20" id="nrCarteira" name="nrCarteira" tabindex="3"
                   value="<?php print isset($_SESSION['dados']['NR_CARTEIRA']) ? $_SESSION['dados']['NR_CARTEIRA'] : ''; ?>"/>
            <input type='button' id='btnConsultarCarteira' name='btnConsultarCarteira' value='Carregar'/>
        </div><!-- #node3 end -->

        <div id="node4">
            <input type="text" size="10" maxlength="10" name="validadeCarteira" tabindex="4"
                   value="<?php print isset($_SESSION['dados']['DT_VALIDADE']) ? $_SESSION['dados']['DT_VALIDADE'] : ''; ?>"
                   disabled/>
        </div><!-- #node4 end DT_VALIDADE_CARTEIRA   DT_VALIDADE -->

        <div id="node7">
            <input type="text" size="1" style="text-transform:uppercase;" style="text-align:center;" maxlength="1"
                   name="atendimentoRN" id="atendimentoRN" tabindex="7"
                   value="<?php print isset($_SESSION['dados']['SN_ATENDIMENTO_RN']) ? $_SESSION['dados']['SN_ATENDIMENTO_RN'] : ''; ?>"
                   onkeydown="validaAtendimentoRN(event);"/>
        </div>
        ...
        <div id="node9">
            <input type="text" size="20" maxlength="15" name="nrCartaoNacionalSaude" tabindex="8"
                   value="<?php print isset($_SESSION['dados']['NR_CNS']) ? $_SESSION['dados']['NR_CNS'] : ''; ?>"/>
        </div><!-- Cartão Nacional de Saúde -->

        <div id="node12">
            <input type="text" size="16" maxlength="15" name="codigoCNES" tabindex="10"
                   value="<?php print isset($_SESSION['dados']['CD_CNES']) ? $_SESSION['dados']['CD_CNES'] : ''; ?>"/>
        </div><!--CNES -->
        ...
        <div id="node11">
            <input type="text" size="60" name="nomeContrato" tabindex="9"
                   value="<?php print $_SESSION['nomefantasia'] ?>" disabled/>
        </div><!-- Contrato -->
        ...
        <div id="node17">
            <select name="cbo" id="cbo" style="width:165px;font-size:small;" tabindex="14">
                <option value="-"> -</option>
                <?php
                foreach ($conn->query("SELECT CD_CBO, DS_CBO FROM TISS_CBO WHERE CD_CBO <> '999999' ORDER BY CD_CBO") as $row) { ?>
                    <option value="<?php print $row['CD_CBO'] ?>"
                        <?php if ((isset($_SESSION['dados']['CD_CBO'])) && ($_SESSION['dados']['CD_CBO'] == $row['CD_CBO'])) {
                            print('SELECTED');
                        } ?>><?php print $row['CD_CBO'] . ' - ' . $row['DS_CBO'] ?></option>
                <?php } ?>
        ...
        <div id="node13">
            <input type="text" style="text-transform:uppercase;" size="55" maxlength="55" name="nomeProfissionalExec"
                   tabindex="10"
                   value="<?php print $_SESSION['tp_credenciado'] == 'F' ? $_SESSION['nomefantasia'] : '' ?>" <?php print $_SESSION['tp_credenciado'] == 'F' ? 'DISABLED' : '' ?>/>
        </div><!-- #node13 end -->

        <div id="node15">
            <input type="text" style="text-transform:uppercase;" size="20" maxlength="15" name="numeroConselho"
                   tabindex="12" value="<?php print $_SESSION['tp_credenciado'] == 'F' ? $_SESSION['nr_crm'] : '' ?>"/>
        </div>

        <div id="node16">
            <input type="text" style="text-transform:uppercase;" size="2" maxlength="2" name="uf" tabindex="13"
                   value="<?php print $_SESSION['tp_credenciado'] == 'F' ? $_SESSION['cd_uf_crm'] : '' ?>"/>
        </div><!-- #node16 end -->

        <div id="node14">
            <select name="conselhoProfissional" id="conselhoProfissional" style="width:80px;font-size:small;"
                    tabindex="14">
                <option value="-"> -</option>
                <?php
                foreach ($conn->query("SELECT CD_CONSELHO, DS_CONSELHO FROM TISS_CONSELHO_PROFISSIONAL WHERE CD_CONSELHO <> 'XXXXXXX'") as $row) { ?>
                    <option value="<?php print $row['CD_CBO'] ?>"
                        <?php if ((isset($_SESSION['dados']['CD_CONSELHO'])) && ($_SESSION['dados']['CD_CONSELHO'] == $row['CD_CONSELHO'])) {
                            print('SELECTED');
                        } ?>><?php print $row['CD_CONSELHO'] . ' - ' . $row['DS_CONSELHO'] ?></option>
                <?php } ?>
            </select>
        ...
        <div id="node24">
            <input type="text" size="20" maxlength="8"
                   value="<?php print isset($_SESSION['dados']['CD_ESPECIALIDADE']) ? $_SESSION['dados']['CD_ESPECIALIDADE'] : '10101012'; ?>"
                   name='cdEspecialidade'/>
        </div><!-- Código Procedimento  -->

        <div id="node25">
            <input type="text" size="20" width="20" maxlength="20" name="nrGuiaPrestador" tabindex="1"
                   value="<?php print isset($_SESSION['dados']['NR_GUIA']) ? $_SESSION['dados']['NR_GUIA'] : ''; ?>"
                   disabled/>
        </div><!-- #node25 end -->

        <div id="node26">
            <input type="number" min="1" step="0.01" size="8" maxlength="8" placeholder='0.00' name="valorProcedimento"
                   tabindex="19"
                   value="<?php print isset($_SESSION['dados']['VL_CONSULTA']) ? $_SESSION['dados']['VL_CONSULTA'] : ''; ?>"/>

        </div>

        <div id="node19">
            <input type="date" size="10" maxlength="10" name="dataAtendimento" tabindex="15"
                   value="<?php print isset($_SESSION['dados']['DATA_ATENDIMENTO']) ? $_SESSION['dados']['DATA_ATENDIMENTO'] : ''; ?>"/>
        </div>
        ...
        <div id="node22">
            <select name="tipoConsulta" id="tipoConsulta" style="width:120px;font-size:small;" tabindex="18">
                ...
                <option value="1" <?php if ((isset($_SESSION['dados']['CD_TIPO_CONSULTA'])) && ($_SESSION['dados']['CD_TIPO_CONSULTA'] == '1')) {
                    print('SELECTED');
                } ?>>1- Primeira Consulta
                </option>
                <option value="2" <?php if ((isset($_SESSION['dados']['CD_TIPO_CONSULTA'])) && ($_SESSION['dados']['CD_TIPO_CONSULTA'] == '2')) {
                    print('SELECTED');
                } ?> >2- Retorno
                </option>
                <option value="3" <?php if ((isset($_SESSION['dados']['CD_TIPO_CONSULTA'])) && ($_SESSION['dados']['CD_TIPO_CONSULTA'] == '3')) {
                    print('SELECTED');
                } ?> >3- Pré-natal
                </option>
                <option value="4" <?php if ((isset($_SESSION['dados']['CD_TIPO_CONSULTA'])) && ($_SESSION['dados']['CD_TIPO_CONSULTA'] == '4')) {
                    print('SELECTED');
        ...
        <div id="node27" tabindex="20">
			<textarea rows="7" cols="150" style="text-transform:uppercase;" maxlength="240"
                      id="observacao"
                      name="observacao"><?php print isset($_SESSION['dados']['DS_OBS']) ? $_SESSION['dados']['DS_OBS'] : ''; ?>
					
			</textarea>
        </div>

        <div id="node30">
            <input type="submit" name="botao" value="Consultar" style="font-size:large;"
                   onclick="return validate('Consultar')"/>
            <input type="submit" name="botao" value="Salvar" style="font-size:large;"
                   onclick="return validate('Salvar')"
...
<script>
        ...
        $("#btnCancelar").click(function () {
            bootbox.confirm("Tem certeza que deseja cancelar o preenchimento da guia ?", function (result) {
                ...
                if (result == true) {
                    ...
                    $('#login').attr('action', 'index.php?pagina=guiaconsulta');
                    $('#login')[0].submit();
                } else {
                    return true;
                }
    ...
    function printDiv() {
        ...
        /*
          var divToPrint=document.getElementById('guiaconsulta');
        ...
        alert(document.getElementById("guiaconsulta").innerHTML);
    ...
    function validate(botao) {
        ...
        vvalorProcedimento = document.getElementsByName("valorProcedimento")[0].value;
        //strMsg ='';
        /*
        if(validarTextArea("dataAtendimento"))
        {
            null;//strMsg += '*A Data de Atendimento é obrigatóriaaaaaaaaaaaaaaMENTEEEEEEE!<br>';
        }
        else
        {
            strMsg += '*Informe a Data do Atendimento!<br>';
            //alert("NÃO Funcionou11!!");
        }
        */
        /*
        var nome = "André";
        ...
        */
        ...
        if (botao == 'Salvar') {
            ...
            /*else if(isNumber(vUf))*/
            else if (typeof vUf == "number") {
                strMsg += '*A UF está preenchida incorretamente!<br>';
            }
            if (vUf.toUpperCase() != 'RJ') {
                strMsg += '*A UF deve ser RJ!<br>';
            }
            if (!vTipoConsulta) {
            ...
            */
            ...
            if (vdataAtendimento == "") {
                strMsg += '*A Data de Atendimento é obrigatória!<br>';
            }
            if (vvalorProcedimento == 0) {
                strMsg += '*O valor do Procedimento é obrigatório!<br>';
            }
        } else {
            if (botao == 'Consultar') {
