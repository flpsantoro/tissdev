...
if (!empty($_POST) || !is_null($nr_protocolo)) {
	...
	if (!is_null($nr_protocolo)) {
		$sql = "SELECT CPF_CGC, NR_PROTOCOLO_RECEBIMENTO, NR_LOTE, DECODE(CD_STATUS_PROTOCOLO, 0, 'Erro', 1, 'Aguardando', 2, 'Em analise', 3, 'Finalizado', 4, 'Excluído') DS_STATUS, TO_CHAR(DT_RECEBIMENTO, 'DD/MM/YYYY') DT_RECEBIMENTO FROM TISS_TRANSACAO_PROTOCOLO ";
		$where[] = "NR_PROTOCOLO_RECEBIMENTO = ". intval($nr_protocolo);
	} else {
...
?>
			<tr>
				...
				<td><a href="guias.php?acao=ver&nr_protocolo=<?php print $row['NR_PROTOCOLO_RECEBIMENTO'] ?>" target="_blank"><?php print $row['NR_PROTOCOLO_RECEBIMENTO'] ?></a></td>
...
<?php	foreach ($conn->query('SELECT NR_PROTOCOLO_RECEBIMENTO FROM TISS_TRANSACAO_PROTOCOLO WHERE CD_STATUS_PROTOCOLO <> 0 AND CPF_CGC = '. $row['CPF_CNPJ']) as $row) { ?>
