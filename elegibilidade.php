...
if (!empty($_POST['nome_civil'])) {	
	$stmt = $conn->prepare('
		SELECT NM_USUARIO, NVL(NOME_SOCIAL, NVL(NOME_AFETIVO,NM_USUARIO)) NOME_SOCIOAFETIVO, 
		       NR_MATRICULA, SEXO, NM_PLANO PLANO
	      FROM IASM.IASM_USUARIO
		 WHERE NM_USUARIO = :nome_civil
	...
	$stmt = $conn->prepare('
		SELECT NM_USUARIO, NVL(NOME_SOCIAL, NVL(NOME_AFETIVO,NM_USUARIO)) NOME_SOCIOAFETIVO,
		       NR_MATRICULA, SEXO, NM_PLANO PLANO
		  FROM IASM.IASM_USUARIO
		 WHERE ((NOME_SOCIAL= :nome_socioafetivo) or 
		        (NOME_AFETIVO = :nome_socioafetivo))
		   AND DT_EXCLUSAO IS NULL');
...
?>
	<?php if (!is_null($nome_socioafetivo)) { ?>
		<h2><font color="red"> <?php print $nome_socioafetivo ?></font></h2>
	<?php } else { ?>
